...
$articlesRows = select('SELECT article FROM Products ORDER BY article');
$articles = array_column($articlesRows, 'article');
...
if ($isEdit) {
    $order = select_one_prepared('SELECT * FROM Orders WHERE id = ?', 'i', $id);
    if (!$order) {
        exit('Заказ не найден.');
    }
    $items = select_prepared('SELECT product_article, quantity FROM OrderItems WHERE order_id = ?', 'i', $id);
    $lines = [];
    ...
    if (!$errors) {
        ...
        try {
            if ($isEdit) {
                ...
                execute_prepared('DELETE FROM OrderItems WHERE order_id = ?', 'i', $id);
                $orderId = $id;
            } else {
                execute_prepared(
            ...
            foreach ($parsedItems as [$art, $qty]) {
                execute_prepared(
...
<form class="panel" method="post">
    ...
    <div class="actions">
        ...
        <a class="btn" href="/frontend/orders.php">Назад</a>
    </div>
</form>
<?php layout_footer(); ?>
