...
$cats = select('SELECT id, name FROM Categories ORDER BY name');
$mans = select('SELECT id, name FROM Manufacturers ORDER BY name');
$units = select('SELECT id, name FROM Units ORDER BY name');
$supplierName = '';

if ($isEdit) {
    $product = select_one_prepared('SELECT * FROM Products WHERE article = ?', 's', $article);
    if (!$product) {
        exit('Товар не найден.');
    }
    $_SESSION['editing_product'] = $article;
    $supRow = select_one_prepared('SELECT name FROM Suppliers WHERE id = ?', 'i', (int)$product['supplier_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['photo']['tmp_name'])) {
        ...
        if (!$info) {
            ...
            if (!save_resized_product_image($_FILES['photo']['tmp_name'], $target)) {
                $errors[] = 'Не удалось обработать и сохранить изображение (макс. 300×200 px).';
    ...
    if (!$errors) {
        $product['supplier_id'] = resolve_supplier_id($supplierName);
        if ($isEdit) {
            execute_prepared(
            ...
            do {
                ...
                $chk = select_one_prepared('SELECT 1 AS ok FROM Products WHERE article=?', 's', $newArticle);
            } while ($chk);
...
<form class="panel" method="post" enctype="multipart/form-data">
    ...
    <label>Поставщик
        <input type="text" name="supplier" value="<?= htmlspecialchars($supplierName) ?>" required>
    </label>
    <label>Производитель
    ...
    <label>Фото товара (сохраняется до 300×200 px)
    ...
    <div class="actions">
        ...
        <a class="btn" href="/frontend/view_1.php?cancel_edit=1">Назад</a>
    </div>
</form>
<?php
layout_footer();
